...
DROP POLICY IF EXISTS "Users can manage own position settings" ON public.position_settings;
...
CREATE TRIGGER update_positions_updated_at BEFORE UPDATE ON public.positions
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
DROP TRIGGER IF EXISTS update_position_settings_updated_at ON public.position_settings;
CREATE TRIGGER update_position_settings_updated_at BEFORE UPDATE ON public.position_settings
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE INDEX IF NOT EXISTS idx_scan_reports_user ON public.scan_reports (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_scan_reports_kind ON public.scan_reports (user_id, kind, created_at DESC);
...
DROP POLICY IF EXISTS "Users manage own scan reports" ON public.scan_reports;
